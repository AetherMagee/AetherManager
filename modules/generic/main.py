...
import database.service.srv as db

@logger.catch
@bot.on(events.NewMessage(pattern="/status"))
async def status(msg):
    logger.debug(f"Got /status command from ID{msg.sender.id} CHID{msg.chat.id}")
...
async def setlang(msg):
    logger.debug(f"Got /setlang command from ID{msg.sender.id} CHID{msg.chat.id}")
    langcode = msg.raw_text.replace("/setlang ", "").lower()
    userInfo = db.getUser(msg.sender.id, createNewIfNone = True)
    if len(langcode) == 2 and langcode in availibleLangCodes:
